...
#prepare data for Figure 1
    ...
    if (nrow(data.delta2[data.delta2$TS01==0,]) >0 ) data.delta2[data.delta2$TS01==0,]$TS01 <- NA
    ...
    if (nrow(data.delta5[data.delta5 $TS01==0,]) >0 ) data.delta5[data.delta5 $TS01==0,]$TS01 <- NA
...
#plot figure for delta=1.2
    ...
    lines(data.delta2 $b, data.delta2 $NashE, type="l", lwd=4, col="red")
    lines(data.delta2 $b, data.delta2 $NashR, type="l", lwd=4, col="blue")
    axis(side=1, at=seq(0,1,0.1), labels=seq(0,1,0.1))
    axis(side=2, at=seq(0,20,2), labels=seq(0,20,2))
    title(main="(a) Firms' Strategic Choices")

    plot(xrange, yrange, type="n", xlab="b", ylab=expression(paste("VOI (", sigma^2, ")")) , xaxt="n", yaxt="n")
    lines(data.delta2 $b, data.delta2 $TS01, type="l", lwd=4, col="darkolivegreen")
    lines(data.delta2 $b, data.delta2 $TS12, type="l", lwd=4, col="darkolivegreen")
    axis(side=1, at=seq(0,1,0.1), labels=seq(0,1,0.1))
    axis(side=2, at=seq(0,20,2), labels=seq(0,20,2))
...
#plot figure for delta=1.5
    ...
    plot(xrange, yrange, type="n", xlab="b", ylab=expression(paste("VOI (", sigma^2, ")")) , xaxt="n", yaxt="n")
    lines(data.delta5 $b, data.delta5 $NashE, type="l", lwd=4, col="red")
    lines(data.delta5 $b, data.delta5 $NashR, type="l", lwd=4, col="blue")
    axis(side=1, at=seq(0,1,0.1), labels=seq(0,1,0.1))
    axis(side=2, at=seq(0,20,2), labels=seq(0,20,2))
    title(main="(a) Firms' Strategic Choices")

    plot(xrange, yrange, type="n", xlab="b", ylab=expression(paste("VOI (", sigma^2, ")")) , xaxt="n", yaxt="n")
    lines(data.delta5 $b, data.delta5 $TS01, type="l", lwd=4, col="darkolivegreen")
    lines(data.delta5 $b, data.delta5 $TS12, type="l", lwd=4, col="darkolivegreen")
    axis(side=1, at=seq(0,1,0.1), labels=seq(0,1,0.1))
    axis(side=2, at=seq(0,20,2), labels=seq(0,20,2))
    title(main="(b) Socially Efficient Strategies")
